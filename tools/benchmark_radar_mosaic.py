...
Every tile computes transient geometry. Decode, QC and PNG encoding are
...
from lib.radar_mosaic import quality_control, render_mosaic
...
def main():
    ...
    print('Sites z median_ms p95_ms')
    for count, candidates in ((1, [scan]), (4, scans)):
        for z in (8, 9, 10):
            ...
            values.sort()
            print(count, z, round(statistics.median(values), 3),
                  round(values[int(.95*(args.iterations-1))], 3))
