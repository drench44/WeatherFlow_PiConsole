#!/usr/bin/env python3
"""Benchmark the actual v4.1 kiosk through localhost CDP; print one JSON result.

Run on the Pi: python3 tools/benchmark_radar_kiosk.py > radar-v41-pi.json
...
BENCH = r"""(async()=>{
 ...
 const out={userAgent:navigator.userAgent,theme:radarBaseStyle.theme,frames:[],memory:[],baseDrawMs:[],fetches:[],decodeMs:[]};
 ...
 try{
  ...
  phase='pan';pointer('pointerdown',901,400,245);for(let i=1;i<=30;i++){pointer('pointermove',901,400+200*i/30,245);await sleep(34);}await sleep(40);pointer('pointerup',901,600,245);await sleep(800);
  radarGestureCancel();radarCameraSet(origin);radarSettle();await sleep(1000);
  phase='pinch';const focal={x:178,y:145};pointer('pointerdown',902,138,145);pointer('pointerdown',903,218,145);
  ...
  pointer('pointerup',902,focal.x-40*2**.63,focal.y);pointer('pointerup',903,focal.x+40*2**.63,focal.y);await sleep(350);
